...
<?php
  ...
  switch ($post_format) {
    ...
    $img_src        = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'big');
          ...
          <?php foreach ($gallery as $item): ?>
            <figure>
              <?php if (isset($item['attachment_id'])):
                echo wp_get_attachment_image( $item['attachment_id'], 'big', array('alt' => esc_attr($item['title'])) );
              endif; ?>
            </figure>
          <?php endforeach; ?>
      </div>
