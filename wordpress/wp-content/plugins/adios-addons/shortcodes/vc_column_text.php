...
function vc_column_text( $atts, $content = '', $id = '' ){
  ...
  $output .= '<div class="item-padd-mob section-text text-block'.$class.$uniqid_class.' '.sanitize_html_class($align).'" '.$id.''.$el_style.'><div class="simple-text '.sanitize_html_class($dp_text_size).'">'.rs_set_wpautop($content).'</div></div>';
  return $output;
}
