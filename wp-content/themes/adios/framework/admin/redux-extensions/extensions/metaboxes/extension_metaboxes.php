        ...
        class ReduxFramework_extension_metaboxes {
            ...
            public function __construct( $parent ) {
                ...
                if ( empty( self::$_extension_dir ) ) {
                    $this->_extension_dir = trailingslashit( str_replace( '\\', '/', get_template_directory().'/framework/admin/redux-extensions/extensions/metaboxes' ) );
                        ...
                        foreach ( $box['post_types'] as $posttype ) {
                            if ( isset( $box['title'] ) ) {
                                ...
                                if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
                                    ...
                                    $title = ucfirst( $posttype ) . " " . __( 'Options', 'adios' );
                                }
                    ...
                    <div class="redux-notices">
                        <?php if ( $sidebar ) : ?>
                            <div class="saved_notice admin-notice notice-blue" style="display:none;">
                                <strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", __( 'Settings Imported!', 'adios' ) ) ?></strong>
                            </div>
                            <div class="redux-save-warn notice-yellow">
                                <strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Settings have changed, you should save them!', 'adios' ) ) ?></strong>
                            </div>
                        <?php endif; ?>
                        <div class="redux-field-errors notice-red">
                            <strong><span></span> <?php echo __( 'error(s) were found!', 'adios' ) ?></strong>
                        </div>
                        <div class="redux-field-warnings notice-yellow">
                            <strong><span></span> <?php echo __( 'warning(s) were found!', 'adios' ) ?>
                            </strong></div>
                    </div>
                    <?php
                        echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . __( 'Expand', 'adios' ) . '</a>';
                        if ( $sidebar ) {
                            ?>
...
//print_r($this->options[$field['id']]);
            ...
            function meta_boxes_show_errors() {
                if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . __( 'error(s) were found!', 'adios' ) . '</strong></p>';
                    echo '</div>';
                }
                if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . __( 'warnings(s) were found!', 'adios' ) . '</strong></p>';
                    echo '</div>';
                }
